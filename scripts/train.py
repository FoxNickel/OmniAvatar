...
from OmniAvatar.utils.training_utils import (
...
class OmniTrainingModule(DiffusionTrainingModule):
    ...
    def load_model(self):
        ...
        if args.train_architecture == "lora":
            # 注入训练好的lora权重
            print(
                f"Use LoRA: lora rank: {args.lora_rank}, lora alpha: {args.lora_alpha}, pretrained_lora_path: {pretrained_lora_path}"
            )
            ...
            print(
                f"load from {resume_path}, {len(missing_keys)} missing keys, {len(unexpected_keys)} unexpected keys"
            )
    ...
    def forward_preprocess(self, data):
        ...
        # TODO 这里就是要将输入数据转换为模型需要的格式和参数，最终返回模型需要的所有东西
        # TODO 这里抄inference里面的forward
        print(f"[train.py] forward_preprocess -> data keys: {data.keys()}")
        audio_path = data["audio_path"]
        audio_path = data["audio_path"]
        image_path = data["image_path"]  # 视频首帧图
        ...
        # import deepspeed
        # checkpointing = deepspeed.checkpointing.checkpoint
    ...
    # data是通过Dataset加载的，inputs是经过forward_preprocess处理的
    # 这里的inputs是一个字典，包含了所有需要的输入参数
    # 例如：{"prompt": "a cat", "input_video": video_tensor}
    def forward(self, data, inputs=None):
        if inputs is None:
            inputs = self.forward_preprocess(data)
        print(f"[train.py] forward -> inputs keys: {inputs.keys()}")
